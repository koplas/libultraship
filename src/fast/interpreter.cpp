...
void Interpreter::GfxSpTri1(uint8_t vtx1_idx, uint8_t vtx2_idx, uint8_t vtx3_idx, bool is_rect) {
    ...
    if (mRdp->viewport_or_scissor_changed) {
        // Direct field comparison instead of memcmp for small structs
        if (mRdp->viewport.x != mRenderingState.viewport.x ||
        ...
        if (mRdp->scissor.x != mRenderingState.scissor.x ||
    ...
    bool use_alpha = ((other_mode_l & (3 << 20)) == (G_BL_CLR_MEM << 20) &&
    ...
    bool invisible =
        (other_mode_l & (3 << 24)) == (G_BL_0 << 24) && (other_mode_l & (3 << 20)) == (G_BL_CLR_MEM << 20);
    ...
    // Branchless shader options computation
    cc_options |= (use_alpha ? SHADER_OPT(ALPHA) : 0)
                | (use_fog ? SHADER_OPT(FOG) : 0)
                | (texture_edge ? SHADER_OPT(TEXTURE_EDGE) : 0)
                | (use_noise ? SHADER_OPT(NOISE) : 0)
                | (use_2cyc ? SHADER_OPT(_2CYC) : 0)
                | (alpha_threshold ? SHADER_OPT(ALPHA_THRESHOLD) : 0)
                | (invisible ? SHADER_OPT(INVISIBLE) : 0)
                | (use_grayscale ? SHADER_OPT(GRAYSCALE) : 0)
                | (mRdp->loaded_texture[0].masked ? SHADER_OPT(TEXEL0_MASK) : 0)
                | (mRdp->loaded_texture[1].masked ? SHADER_OPT(TEXEL1_MASK) : 0)
                | (mRdp->loaded_texture[0].blended ? SHADER_OPT(TEXEL0_BLEND) : 0)
                | (mRdp->loaded_texture[1].blended ? SHADER_OPT(TEXEL1_BLEND) : 0);
    if (shader.enabled) {
        cc_options |= SHADER_OPT(USE_SHADER) | (shader.id << 17);
    }
