...
std::shared_ptr<ResourceInitData> ResourceLoader::ReadResourceInitDataLegacy(const std::string& filePath,
    ...
    static constexpr uint8_t sKtX2Magic[] = { 0xAB, 0x4B, 0x54, 0x58, 0x20, 0x32, 0x30, 0xBB, 0x0D, 0x0A, 0x1A, 0x0A };
    if (fileToLoad->Buffer->size() >= sizeof(sKtX2Magic) &&
        std::memcmp(fileToLoad->Buffer->data(), sKtX2Magic, sizeof(sKtX2Magic)) == 0) {
